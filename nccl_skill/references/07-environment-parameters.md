...
- `repos/nccl/src/include/param.h`: `NCCL_PARAM(name, env, default)` macro.
- `repos/nccl/src/include/param/param.h`: newer typed parameter definitions.
- `repos/nccl/src/param/param.cc`: public parameter registry flags.
- `repos/nccl/src/param/c_api.cc`: public parameter API glue.
- `repos/nccl/src/nccl.h.in`: `ncclParam*` public APIs.
- Parameter definitions across `repos/nccl/src/**/*.cc`.
...
| `NCCL_RUN_DIAGNOSTICS` | `diagnostics.cc`, `0` | run NCCL comm-init diagnostics |
| `NCCL_RUN_RAS_DIAGNOSTICS` | `ras/ras.cc`, `0` | run passive RAS diagnostics |
...
| `NCCL_CTA_POLICY` | `init.cc`, unset | CTA policy override: legacy `0/1/2` or combinable `DEFAULT\|EFFICIENCY\|ZERO` tokens; `ZERO` (`NCCL_CTA_POLICY_ZERO` in `nccl.h.in`) selects the zero-SM collective path (copy-engine/RMA offload instead of SM-resident kernels) |
...
| `NCCL_NTHREADS` | `tuning/tuning_general.cc`, `-2` | threads for algorithms/protocols |
| `NCCL_LL128_NTHREADS` | `tuning/tuning_general.cc`, `-2` | LL128 thread count |
...
`tuning/cost_model.cc` (`parseList`) parses prefix-list syntax for algorithm/protocol filters. Examples from source comments:
...
Valid algorithm names come from `ncclAlgoStr` (`init.cc`): `Tree`, `Ring`, `CollNetDirect`, `CollNetChain`, `NVLS`,
`NVLSTree`, `PAT`. `PAT` (NCCL 2.31) applies to AllGather/ReduceScatter with the Simple protocol only
(`config/algorithm_registry.cc`); multi-RPN PAT stages intra-node transfers through NVLS
(`tuning/tuning_general.cc`, `enqueue/task_prep/task_posttuning.cc`). Example:

```bash
export NCCL_ALGO="PAT;allreduce:ring,tree"
```
...
| `NCCL_PAT_ENABLE` | `tuning/pat.cc`, `2` | PAT enable/auto |
...
| `NCCL_NET_OVERHEAD` | `tuning/ring.cc`, `-2` | network post overhead model |
...
| Variable | Source/default | Use |
|---|---|---|
| `NCCL_CUMEM_ENABLE` | `misc/cudawrap.cc`, `-2` | cuMem API usage toggle; `-2` means auto-detect cuMem/VMM support |
...
| `NCCL_GIN_IB_TC` | `transport/net_ib/gin.cc`, `-1` | GIN IB traffic class override (falls back to `NCCL_IB_TC`) |
| `NCCL_GIN_NCONNECTIONS` | `gin/gin_host.cc`, `-2` | GIN connection count |
| `NCCL_GIN_PROXY_NTHREADS` | `gin/gin_host.cc`, `1` | GIN proxy progress threads (capped at `NCCL_GIN_MAX_CONNECTIONS`) |
...
| `NCCL_RMA_DISABLE` | `rma/rma.cc`, `0` | disable the RMA proxy path |
| `NCCL_SYM_TMA_ENABLE` | `sym_kernels.cc`, `1` | symmetric-kernel TMA path; requires compute capability >= 100 |

Note: `NCCL_GIN_RESOURCE_SHARING_THREAD` is not an environment variable. It is a value of the Device-API enum
`ncclGinResourceSharingMode` (`include/nccl_device/gin/gin_device_common.h`, alongside `..._GPU` and `..._CTA`),
selected per `ncclGin` instance at construction time for per-thread resource sharing.
