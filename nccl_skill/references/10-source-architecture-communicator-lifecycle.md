...
- `repos/nccl/src/include/comm.h`: central host-side runtime structures.
- `repos/nccl/src/init.cc`: communicator init, split/shrink/grow/finalize/destroy/abort.
- `repos/nccl/src/bootstrap.cc`: rank bootstrap and unique ID exchange.
- `repos/nccl/src/channel.cc`: channel helpers.
- `repos/nccl/src/enhcompat.cc`: enhanced compatibility/version shims.
- `repos/nccl/src/devcomm/*`: Device API compatibility layers for older ABI versions.
- `repos/nccl/src/include/core.h`, `checks.h`, `argcheck.h`: common API checking and return-code patterns.
