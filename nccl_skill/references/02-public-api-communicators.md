...
- `repos/nccl/src/nccl.h.in`: public C API template.
- `repos/nccl/src/init.cc`: implementation of communicator lifecycle APIs.
- `repos/nccl/src/dev_runtime.cc`: Device API host entry points such as `ncclCommQueryProperties`.
- `repos/nccl/docs/examples/01_communicators/*`: initialization examples.

## Version macros

The generated public header exposes (values from `repos/nccl/makefiles/version.mk`, NCCL 2.31.2):
...
Introduced in 2.31 (`ncclCollConfig_v23100` in `repos/nccl/src/nccl.h.in`), `ncclCollConfig_t`
customizes an individual collective call. It is versioned and append-only like `ncclConfig_t` and must
be initialized with `NCCL_COLLCONFIG_INITIALIZER`:

```c
ncclCollConfig_t collConfig = NCCL_COLLCONFIG_INITIALIZER;
collConfig.maxCTAs = 8;
collConfig.algSelection = "RING";
ncclAllReduceConfig(sendbuff, recvbuff, count, datatype, op, comm, stream, &collConfig);
```

Fields from `ncclCollConfig_t`:

| Field | Purpose |
|---|---|
| `ext` | linked list of `ncclConfigExt_t` vendor-specific key-value options; the official library ignores every extension, vendors pick a unique non-zero `key.vendorId` |
| `minCTAs`, `maxCTAs` | bound channels/CTAs for this call; `maxCTAs` must be `<= comm->maxCTAs` |
| `nvlsCTAs` | NVLS-pool-specific channel cap |
| `cgaClusterSize` | CUDA thread-block-cluster size (Hopper+); must be identical for all collectives in the same group, otherwise undefined behavior |
| `algSelection` | per-call algorithm selection string; NULL/empty means automatic |
| `forceAlgSelection` | default 1: an unsatisfiable `algSelection` is an error; 0 falls back to automatic selection |
| `CTAPolicy` | `NCCL_CTA_POLICY_*` bitmask overriding the communicator-level policy for this call |
| `userProfilerTag` | opaque value delivered verbatim to profiler plugins with this call's events; values with the MSB set are reserved by NCCL |

The same configuration must be set on every rank; NCCL validates it only locally. Do not manually fill
`size`, `magic`, or `version`.

### `nccl*Config` entry points

Every collective has a variant taking a trailing `const ncclCollConfig_t* config`
(`repos/nccl/src/nccl.h.in`): `ncclAllReduceConfig`, `ncclBroadcastConfig`, `ncclReduceConfig`,
`ncclAllGatherConfig`, `ncclReduceScatterConfig`, `ncclAlltoAllConfig`, `ncclGatherConfig`,
`ncclScatterConfig`. Passing `config == NULL` is equivalent to the plain API. Point-to-point
`ncclSend`/`ncclRecv` have no config variant.

### `algSelection` syntax

Parsed by `ncclAlgParse` in `repos/nccl/src/config/algorithm_parser.cc`; whitespace is ignored and
matching is a case-insensitive prefix against the registry names in
`repos/nccl/src/config/algorithm_registry.cc` (e.g. `"RING"` selects `RING_LL`/`RING_LL128`/
`RING_SIMPLE`, and `SYMK_*` names cover the symmetric kernels):
...
Only algorithms valid for the called collective are kept; if nothing matches, `forceAlgSelection`
decides between `ncclInvalidArgument` and falling back to automatic.

### Per-call `CTAPolicy` resolution

Valid values are `NCCL_CTA_POLICY_DEFAULT` (0x00), `NCCL_CTA_POLICY_EFFICIENCY` (0x01), and
`NCCL_CTA_POLICY_ZERO` (0x02) from `repos/nccl/src/nccl.h.in`. Resolution order in
`repos/nccl/src/config/collconfig.cc` (`ncclCollConfigResolveCTAPolicy`): an `NCCL_CTA_POLICY`
environment override (folded into the comm config at init in `repos/nccl/src/init.cc`) wins; otherwise
the per-call value wins over the comm-level value, with `ZERO` taking precedence over `EFFICIENCY` when
both bits are set.
