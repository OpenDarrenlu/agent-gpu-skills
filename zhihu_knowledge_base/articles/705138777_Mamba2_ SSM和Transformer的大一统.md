...
**原文链接**: https://zhuanlan.zhihu.com/p/705138777
...
最近Mamba-2被ICML接收，实现了状态空间模型和注意力机制的大一统，不仅拥有较强的模型表现，还实现了性能的8倍优化。我最近正好在研读这方面的工作，认真学习了一下技术报告和原文，这里就做一些记录。原文真的太长了，所以本文力求提取核心思想，有错误之处，请大家帮忙指出和改正。但是原文真的非常值得一读！从理论基础到算法设计，到模型结构的设计，到系统性能优化，真的是十分全面和系统，值得反复品味和学习。
...
Nvidia做了更多的验证，分为两个方面（参考An Empirical Study of Mamba-based Language Models）：
...
\begin{aligned} h_t & ={A} h_{t-1}+{B} x_t \\ y_t & =C^{T} h_t \end{aligned} \\

本质是定义了一个映射 x \in \mathbb{R}^{\mathrm{T}} \rightarrow y \in \mathbb{R}^{\mathrm{T}} ， x_t,y_t 都是标量，中间隐藏状态 h_t 是 N 维向量。参数 A \in \mathbb{R}^{(\mathrm{T}, \mathrm{N}, \mathrm{N})}, B \in \mathbb{R}^{(\mathrm{T}, \mathrm{N})} \text {, } C \in \mathbb{R}^{(\mathrm{T}, \mathrm{N})} 。

为什么是结构化SSM？

结构化矩阵的定义：

一般矩阵 M \in \mathbb{R}^{(\mathrm{T}, \mathrm{T})} 需要 T^{2} 个参数来表示，并且需要O(T^{2}) 时间来执行矩阵-向量乘法等基本操作。结构化矩阵是指：
...
以上的状态表示是连续的，而在实际的系统中状态通常是通过采样来获取的，因此需要进行离散化处理。Mamba2 paper 为了简化，就不单独为离散的矩阵设置单独的符号表示了。所以在本文中，我们就直接认为A，B已经做好了离散化处理。也就是说， (A, B) 是通过参数 (\overline{A} ,\overline{B}) 以及step size \Delta 生成的。连续的参数 (\Delta,\overline{A} ,\overline{B}) 通过公式 A=f_{A}(\Delta,\overline{A}) 和 B=f_{B}(\Delta,\overline{B}) 转成离散参数 (A, B)，其中 (f_{A},f_{B}) 为离散准则。

离散化步长的说明：对于连续信号保持一个特定值的时间可以由一个可学习的参数表示，称为步长 \Delta 。根据连续信号和步长采样，就可以得到离散输出。
...
\begin{aligned} h_t & =A h_{t-1}+B x_t \\ y_t & =C^{\top} h_t \end{aligned}\\

也就是线性时不变SSM，可以表达为卷积形式和循环形式；卷积形式可以实现并行，输出 y_{t} 不依赖于 y_{t-1} 只取决于卷积核和输入 x_{t}。
卷积形式可以用来训练，我们已经拥有了全部的输入token，实现并行计算；循环形式可以用来推理，对每个生成的token，它的只需要考虑之前的隐藏状态和当前的输入即可，计算量和内存占用量都是常量。

S4 的局限：

系统的行为由状态变量的演化和外部控制信号的影响决定。状态变量是系统的内部表示，可以捕获系统的动态特性，通过 A 来控制；外部控制信号的影响通过 B 来控制；系统的状态转化为最终的输出通过矩阵C 来控制。
LTI表示SSM参数A、B和C对于所有时间步长都是固定的。无论给SSM的序列是什么，可学习的参数 A,B,C 的值随着新token的传入都保持不变。这样就得到了一个不感知内容的静态表示。
...
线性注意力的因果掩码 (Causal) Linear Attention：
...
使用tensor contractionor einsum notation来表示 Y=(L\circ QK^{T})V ，也就是二次形式的结构化掩码矩阵：
...
L=\left[\begin{array}{ccccl} 1 & & & & \\ a_{1} & 1 & & & \\ a_{2}a_{1} & a_{2} & 1 & & \\ \vdots & \vdots & \ddots & \ddots & \\ a_{T-1}...a_{1} & a_{T-1}...a_{2} & a_{T-1} & 1 \end{array}\right]\\[定义]：这种形式的矩阵为1-SS (1-semiseparable）半可分离矩阵 。半可分离矩阵的另一种表征是它们的结构秩属性，它表示包含在下三角形部分的每个子矩阵都是低秩的。
...
SSD使用统一的理论将 SSM 和 Attention 联系了起来，其交集的部分即 SSD 模型，在该理论下 Linear Attention 和 RetNet 只是其一种特殊形式。
...
(橙色) 每个对角块是一个小的半可分离矩阵，可以通过SSD的二次形式，也就是类似Attention的方式来计算矩阵乘；不同的块之间可以并行计算；
(绿色) 只有T/Q 个不同的block，大部分的block都是share的（可以看到第一列的两个block是一样的）。可以通过batched matmul来计算；
(黄色) 黄色部分自己形成了一个1-SS矩阵，这部分计算等同于SSM scan；
(蓝色) 和绿色部分类似，可以通过batched matmul来计算。
SSD Algorithm: Chunking and State Passing

这里就是通过状态传递的视角来理解SSD。我们把序列分成size为Q的blocks/chunks，这里的chunk就会有不同的状态：

Intra-chunk outputs: 计算每个chunk的局部输出：假设初始状态是0，计算其输出状态；
Chunk states: 计算出每个chunk的最终状态；
Pass states: 通过parallel或者sequential scan等方法计算所有chunk的递归状态，也就是考虑了之前时间点所有状态的最终状态；
Output states: 对每个chunk，给定初始状态（step3 得到的），计算对从初始状态对输出的贡献。
Code

我们来看一下最小版本的实现代码：AI_analysis/mamba2_ssd.ipynb at main · ifromeast/AI_analysis (github.com)
...
由于SSM对数值十分敏感，这个操作需要做减法操作，这就会引入Catastrophic_cancellation问题，简单来说，两个大的数值十分接近，相对差值很小；但是他们减去同一个数后，变成小的数值了，相对差值就变大了。
...
多头选择性状态空间模型（Multihead SSM (MHS)）/多头注意力（Multihead Attention (MHA) Pattern）
...
State Space Duality (Mamba-2) Part II - The Theory | Goomba Lab

State Space Duality (Mamba-2) Part III - The Algorithm | Goomba Lab

State Space Duality (Mamba-2) Part IV - The Systems | Goomba Lab

Mamba: The Easy Way (jackcook.com)

Mamba: The Hard Way (srush.github.io)

https://newsletter.maartengrootendorst.com/p/a-visual-guide-to-mamba-and-state

Megatron-LM/examples/mamba at ssm · NVIDIA/Megatron-LM (github.com)

An Empirical Study of Mamba-based Language Models

LLM（廿五）：从控制系统到语言模型 —— Mamba 的前世今生 - 知乎 (zhihu.com)

LLM（ 廿八）：解读 Mamba 2，兼谈 LLM 的统一理论 - 知乎 (zhihu.com)
