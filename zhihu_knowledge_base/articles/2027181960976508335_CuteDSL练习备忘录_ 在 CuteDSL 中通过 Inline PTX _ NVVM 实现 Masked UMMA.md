# CuteDSL练习备忘录: 在 CuteDSL 中通过 Inline PTX / NVVM 实现 Masked UMMA

**作者**: NobodyLove the life you live.

**原文链接**: https://zhuanlan.zhihu.com/p/2027181960976508335
...
目前实现了两条路径来发射带 mask 的 tcgen05.mma：
...
Weight-stationary 变体有对应的 nvvm.tcgen05_mma_ws op，但该 op 不接受 write_disable_mask 参数，无法生成带 mask 的 .ws. 变体。 下面是 inline_ptx asm 的 ws mma 形式：
...
128-bit mask 由 4 个 uint32 组成，我这次主要用到的是 M==64 的 TMEM ACC Layout F
...
CuteDSL inline ptx / nvvm 有什么用
1. CuteDSL 的”逃生舱” 机制

CuteDSL 提供了 两个层次的底层访问能力：

llvm.inline_asm：嵌入任意 PTX 汇编字符串，用于 NVVM op 尚未覆盖的指令变体
nvvm.* 原生 op：直接映射到 PTX 指令的 MLIR op，理论上编译器可以做更好的优化
...
从 PTX ISA 规范中提取指令编码细节（如 descriptor 位域、mask 布局）
定位 CuteDSL/MLIR 中对应的底层 op 和类型系统
完成你需要的非标 API 功能
...
api 封装： https://github.com/inclusionAI/cuLA/blob/cavan.inline_ptx/cula/kda/ptx_umma_masked.py 主要代码基本是仿照 tilelang cutedsl backend 示例构造出来的

测试： https://github.com/inclusionAI/cuLA/blob/cavan.inline_ptx/tests/test_ptx_umma_masked.py
