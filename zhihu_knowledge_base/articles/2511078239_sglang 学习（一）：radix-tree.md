# sglang 学习（一）：radix-tree

**作者**: weishengying

**原文链接**: https://zhuanlan.zhihu.com/p/2511078239
...
本节学习一下 radix-tree 的实现细节， sglang 对应的论文地址 sglang

对应的测试代码仓库：测试代码

测试代码 src 目录中有三个文件，都是从 sglang 中 copy 下来的，方便独立的测试和阅读源码。对应官方 mem_cache 目录下， 这里 copy 的是 commit:c996e8ccd415f6e1077ace5bc645d19a8dd40203 中的代码，增加了很多详细的注释，对理解有一些帮助。
...
这个需求更精准的描述就是：把一些计算过 kv 值的 token 缓存起来，下次输入一个新的 prompt 时，尽可能的找到前 n 个已经被缓存了kv值的 token （姑且叫做前向最大匹配），将这部分 token 的 kv 值直接从 kv_pool 中取出来用，然后只需计算出剩余token 的 kv 值。
...
将该 prompot 插入 radix-tree 中（实际是插入这些 prompot 的 token_id 以及这些 token_id 对应的 kv_indices 值） 得到的字典树如下：（使用 pretty_print api 打印出来的结构如下）

27 hello, what your first name r=0
#tokens: 27
字典树第一次insert

需要注意 node 子节点的储存方式，是一个map，key 是子节点储存的 token 的第一个（对应上图中的 “h”），方便快速匹配查找

2. 输入 prompt = "hello, what your second name"

将第二个输入的 prompot 插入 radix-tree 中 得到的字典树如下：（使用 pretty_print api 打印出来的结构如下）
...
第二次输入的 prompt 插入 radix-tree 时，节点发生了一次 split：

split 操作的目的：为了后续输入的 prompt 能够更快速地找到最大的匹配的前 n 个 token
...
按照叶子节点最后一次的访问时间构建一个最小堆，然后从堆中弹出最小的元素，如果该元素被其他请求使用，则跳过，否则释放该元素，并删除该元素（叶子节点） 。
...
总结一下：radix-tree 设计的目的就是记录和快速查找缓存了的 kv 的token (实际存储在kv pool中，分散储存)，整体设计追求的就是两点：快速查找；最大匹配，这也是 split 的原因，更多细节可以查看仓库代码中的代码注释和测试代码。
