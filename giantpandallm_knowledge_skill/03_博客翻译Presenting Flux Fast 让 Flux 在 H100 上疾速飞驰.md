...
**原文链接**: https://mp.weixin.qq.com/s/KRKqZdcTjfbAmhIPYDXwTQ

**下载时间**: 2026-06-05 22:12:20

---

博客来源：https://pytorch.org/blog/presenting-flux-fast-making-flux-go-brrr-on-h100s/ By Joel Schlosser (Meta), Christian Puhrsch (Meta), and Sayak Paul (Hugging Face)June 25, 2025 。这篇博客介绍了Meta和Hugging Face团队如何使用原生PyTorch代码将Flux.1-Schnell和Flux.1-Dev模型在H100 GPU上实现约2.5倍的加速优化。主要优化技术包括：使用
torch.compile
配合
fullgraph=True
和
max-autotune
模式、合并attention计算的q/k/v投影、采用Flash Attention v3和float8量化、应用特定的Inductor调优标志，以及通过
torch.export
+AOT编译+CUDAGraphs消除框架开销。博客还特别指出了消除CPU-GPU同步点对torch.compile性能的重要性，并展示了在保持图像质量基本不变的前提下显著提升推理性能的效果。公众号翻译此文仅做科普和知识传播，侵删。
...
优化概述
Diffusers 库中提供的 pipeline 尽可能地对
torch.compile
友好。这意味着：
尽可能避免计算图中断
尽可能避免重新编译
...
torch.channels_last
内存格式
Flash Attention v3 (FA3)(https://pytorch.org/blog/flashattention-3/) 配合输入到
torch.float8_e4m3fn
dtype 的（非缩放）转换
通过 torchao 的
float8_dynamic_activation_float8_weight
进行动态 float8 激活量化和 Linear 层权重量化
...
torch.export
+ Ahead-of-time Inductor (AOTI) + CUDAGraphs
...
另一个关键点是，具有 Hopper 架构的 GPU 通常成本很高。因此，为了在消费级 GPU 上提供合理的速度-内存权衡，Diffusers 库中也提供了其他（通常与
torch.compile
兼容的）选项。我们邀请您在这里(https://huggingface.co/docs/diffusers/main/en/optimization/memory)和这里(https://huggingface.co/docs/diffusers/main/en/optimization/fp16)查看它们。
...
1. https://mmbiz.qpic.cn/mmbiz_png/SdQCib1UzF3uVibeeIJGW2RKIzeqticMLyehXU8mVysnvAuVczAZFgc1ibY3r1dianpDOw1QC8FTGb9Vuzw7BuVG30A/640?wx_fmt=png&from=appmsg
2. https://mmbiz.qpic.cn/mmbiz_png/SdQCib1UzF3uVibeeIJGW2RKIzeqticMLyeF6cNknQKLBpl5ibuwSf9jvynicHTib68Doxo4JgfVxvISLkInycjU5Biag/640?wx_fmt=png&from=appmsg
3. https://mmbiz.qpic.cn/mmbiz_png/SdQCib1UzF3uVibeeIJGW2RKIzeqticMLyeaHVJlnL66xuzIrvnv1jzFIicHTyOYbKTTBhbJfoCuwE5QCgV30JRr7g/640?wx_fmt=png&from=appmsg
